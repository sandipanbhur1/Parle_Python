...
def createMultiFileTypeLoad(rawHiveDbName,landingAreaFolder,tempStagingFolder,fileFormat,FileKey,delimiter,serde,skipHeader,res):
   ...
   for i in headerNextLineRemoved:
       for index, row in res.iterrows():
           if row['Column_Name'] == i and row['Table_Name'] == j:
		       interim=row['Column_Name']+" "+row['Refined_Data_Type']+","
		       query_input.append(interim)
		    else:
			   interim=row['Column_Name']+"  string,"
			   query_input.append(interim)
           
   
   replaceDelimiterByDatatype = query_input.replace('/','').replace('|','').replace('_','')
   replaceDelimiterByDatatype[-1].replace(',','')
...
if __name__ == '__main__':
  ...
  parser.read('/home/darshana/simple_prod.ini')
