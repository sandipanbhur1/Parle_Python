...
#################################
#Script To Ingest Multiple Tables
#author-gopesh.pandey@antuit.com
#################################
...
def createMultiFileTypeLoad(rawHiveDbName,landingAreaFolder,tempStagingFolder,fileFormat,FileKey,delimiter,serde,skipHeader):
 ...
 for path in files:
   ...
   hdfsFilePath="wasb://str-papl-prod@strpaplprod.blob.core.windows.net/"+tempStagingFolder+"/%s" % filenameWithExt
   ...
   try: 
    ...
    dropBeelineCommand = """beeline -u jdbc:hive2://10.21.1.16:10000  hive hive123 -e '%s'""" % dropQuery
    print "\n" +dropQuery
    createBeelineCommand = """beeline -u jdbc:hive2://10.21.1.16:10000  hive hive123 -e '%s'""" % createQuery
   ...
   try:
    ...
    loadBeelineCommand= """beeline -u jdbc:hive2://10.21.1.16:10000  hive hive123  -e '%s'""" % loadQuery
    
    line = os.popen(loadBeelineCommand,"r")
    result = line.readline()
	
    countQuery = 'SELECT count(*) from '+rawHiveDbName+'.'+table_name_wod 
    countBeelineCommand= """beeline -u jdbc:hive2://10.21.1.16:10000  hive hive123  -e '%s'""" % countQuery
    
    count1 = os.popen(countBeelineCommand,"r")
    list1=[]
    for i in range(4):
        list1.append(count1.readline())
    #result1 = count1.readline()
    Count = list1[3].replace("|", "")
    Count = list1[3].replace("\n", "")
    
    
    f=open('/home/darshana/test_prod.txt','a+')
    f.write(table_name_wod)
    f.write(",")
    f.write(Count)
    f.write("\n")
   except:
...
if __name__ == "__main__":
  ...
  parser.read('/home/darshana/simple.ini')
